    ...
    const assertEvidenceRefs = (refs) => {
        for (const ref of refs ?? []) {
            ...
            if (prefix === undefined || ref.slice(prefix.length).trim().length === 0)
                throw new Error(`workflow smoke evidence reference "${ref}" must use a non-empty file:, diff:, finding:, or task_id: reference`);
            if (prefix !== 'task_id:')
                continue;
            ...
            if ([...tasks.values()].some(task => task.name === taskId))
                throw new Error(`workflow smoke evidence reference "${ref}" used an agent name, but task_id: requires the taskId returned by spawnAgent/runAgent`);
    ...
    const api = {
        ...
        parallel: async (thunks, options) => {
            ...
            for (const thunk of thunks)
                values.push(await thunk());
            void options;
            return values;
        },
        pipeline: async (items, ...stages) => await Promise.all(items.map(async (item, index) => { let value = item; for (const stage of stages)
            value = await stage(value, item, index); return value; })),
...
async function smokeCapsule(capsule, config) {
    ...
    if (literalLaunches > capsule.manifest.maxAgents)
        throw new Error(`workflow source contains ${literalLaunches} static agent launches but manifest.maxAgents is ${capsule.manifest.maxAgents}`);
