...
(() => {
  ...
  const call = async (method, payload) => {
    const envelope = parse(await __dshCall(method, JSON.stringify(payload === undefined ? null : payload)));
    if (!envelope.ok) {
  ...
  const sync = (method, payload) => {
    const envelope = parse(__dshSync(method, JSON.stringify(payload === undefined ? null : payload)));
    if (!envelope.ok) {
      const error = new Error(envelope.error || ('wf.' + method + ' failed'));
      if (envelope.fatal) Object.defineProperty(error, '__workflowFatal', { value: true });
      throw error;
  ...
  const pipeline = async (items, ...stages) => {
    ...
    return await Promise.all(items.map(async (item, index) => {
      try {
        ...
        return value;
      } catch (error) { if (error && error.__workflowFatal) throw error; return null; }
    }));
  };
...
export async function runRestrictedWorkflowScript(options) {
    ...
    try {
        const asyncHandle = vm.newFunction('__dshCall', (methodHandle, payloadHandle) => {
            ...
            let dispatch;
            dispatch = asyncDispatch(method, payload).then(value => settle(deferred, { ok: true, value: jsonClone(value === undefined ? null : value, `wf.${method} result`) }), error => settle(deferred, { ok: false, error: error instanceof Error ? error.message : String(error), fatal: error instanceof WorkflowScriptError && error.fatal })).finally(() => {
