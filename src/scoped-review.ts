...
import type { WorkflowApi, WorkflowModule, WorkflowTaskResult } from './types.js'
...
const DEFAULT_PACKET_BUDGET: ReviewPacketBudget = { maxBytes: 50 * 1024, maxLines: 2_000, maxLineChars: 2_000 }
...
function normalizePacketPath(value: string): string {
...
function classifyFileCategory(filePath: string): 'source' | 'docs' | 'tests' | 'config' | 'other' {
  const normalized = normalizePacketPath(filePath)
  const lower = normalized.toLowerCase()
  const extension = path.posix.extname(normalized).toLowerCase()
  const base = path.posix.basename(normalized)
  if (CONFIG_NAMES.has(base)) return 'config'
  if (normalized.startsWith('docs/') || KEY_DOC_NAMES.has(normalized) || DOC_EXTENSIONS.has(extension)) return 'docs'
  const segments = lower.split('/')
  if (segments.some(segment => TEST_SEGMENTS.has(segment)) || /\.(?:test|spec)\.[^.]+$/u.test(lower)) return 'tests'
...
function areaIdForPath(filePath: string): string {
  ...
  if (['packages', 'clients', 'apps', 'libs', 'services'].includes(parts[0]!) && parts[1] !== undefined) return `${parts[0]}/${parts[1]}`
...
function captureFileDiffs(diff: string): readonly CapturedFileDiff[] {
  ...
  if (starts.length === 0) return [{ path: '(cross-cutting)', partitionKey: 'cross-cutting/other', content: diff }]
...
function rangeSummary(diff: string): readonly string[] {
...
function fitsPacketBudget(value: string, budget: ReviewPacketBudget): boolean {
...
function splitLongLine(line: string, maxLineChars: number): readonly string[] {
  ...
  if (longestMarker.length >= maxLineChars) return Array.from({ length: Math.ceil(line.length / maxLineChars) }, (_, index) => line.slice(index * maxLineChars, (index + 1) * maxLineChars))
...
function chunkEvidence(content: string, budget: ReviewPacketBudget): readonly string[] {
  ...
  for (const line of content.split(/\r?\n/u).flatMap(value => splitLongLine(value, budget.maxLineChars))) {
    ...
    if (current.length > 0 && !fitsPacketBudget(prospective, budget)) flush()
    current.push(line)
    if (!fitsPacketBudget(current.join('\n'), budget)) throw new Error('review packet budget is too small for one evidence segment')
  }
  flush()
...
function groupPackets(files: readonly CapturedFileDiff[], input: ReviewPacketInput, rangeId: string, budget: ReviewPacketBudget): readonly CapturedFileDiff[][] {
  ...
  for (const file of files) {
    ...
    if (current.length > 0 && !fitsPacketBudget(body, budget)) {
      groups.push(current)
      current = [file]
    } else current = prospective
  }
  if (current.length > 0) groups.push(current)
  return groups
}

function validatePacketInput(input: ReviewPacketInput): ReviewPacketBudget {
  for (const [label, value] of [['cwd', input.cwd], ['sessionId', input.sessionId], ['label', input.label]] as const) {
    if (typeof value !== 'string' || value.trim().length === 0) throw new Error(`review packet ${label} must be a non-empty string`)
  }
  if (typeof input.diff !== 'string') throw new Error('review packet diff must be a string')
  for (const [label, value] of [['baseRef', input.baseRef], ['headRef', input.headRef], ['customPrompt', input.customPrompt]] as const) {
    if (value !== undefined && typeof value !== 'string') throw new Error(`review packet ${label} must be a string`)
  }
  if (input.scope !== undefined && !['staged', 'unstaged', 'all', 'compare', 'commit'].includes(input.scope)) throw new Error('review packet scope is invalid')
  if (input.routingRisk !== undefined && !['low', 'medium', 'high'].includes(input.routingRisk)) throw new Error('review packet routingRisk is invalid')
  for (const [label, values] of [['requirements', input.requirements], ['testEvidence', input.testEvidence]] as const) {
    if (values !== undefined && (!Array.isArray(values) || values.some(value => typeof value !== 'string'))) throw new Error(`review packet ${label} must contain strings`)
  }
  const budget = input.budget ?? DEFAULT_PACKET_BUDGET
  if (typeof budget !== 'object' || budget === null) throw new Error('review packet budget must be an object')
  if (![budget.maxBytes, budget.maxLines, budget.maxLineChars].every(value => Number.isSafeInteger(value) && value > 0)) throw new Error('review packet budget must contain positive safe integers')
...
async function writeImmutableFile(filePath: string, content: string): Promise<void> {
  try {
    ...
    if (!stats.isFile() || await readFile(filePath, 'utf8') !== content) throw new Error(`immutable review evidence collision at ${filePath}`)
  }
}

async function ensureContainedDirectory(workspace: string, segments: readonly string[]): Promise<string> {
  ...
  for (const segment of segments) {
    ...
    if (!stats.isDirectory() || stats.isSymbolicLink()) throw new Error(`review packet storage component is not a plain directory: ${next}`)
    current = await realpath(next)
    const relative = path.relative(workspace, current)
    if (relative.startsWith('..') || path.isAbsolute(relative)) throw new Error('review packet directory escapes the workspace')
  }
  return current
}

function freezePacket(value: ReviewPacketMetadata): ReviewPacketMetadata {
  Object.freeze(value.budget)
  for (const chunk of value.evidenceChunks) Object.freeze(chunk)
...
export interface VerifiedPacketReview {
  ...
  readonly audit: { readonly findings: readonly FindingDisposition[] }
  readonly unqualifiedApprovalAllowed: boolean
}

export interface ScopedReviewResult {
  readonly summary: string
  readonly packetResults: readonly { readonly contentHash: string; readonly result: VerifiedPacketReview }[]
}

interface RawReview {
  readonly specVerdict: SpecVerdict
  readonly qualityVerdict: QualityVerdict
  readonly findings: readonly { readonly severity: ReviewSeverity; readonly location: string; readonly claim: string; readonly evidence: string; readonly suggestedFix?: string }[]
  readonly unverifiedRequirements: readonly string[]
}
...
const SUMMARY_SCHEMA = { type: 'object', additionalProperties: false, required: ['summary'], properties: { summary: { type: 'string' } } } as ObjectJsonSchema
const SEVERITY: Record<ReviewSeverity, number> = { low: 0, medium: 1, high: 2, critical: 3 }
...
function packet(value: ReviewPacketMetadata, index: number): ReviewPacketMetadata {
  if (typeof value !== 'object' || value === null) throw new Error(`packets[${index}] must be an object`)
  for (const key of ['packetPath', 'contentHash', 'rangeId', 'partitionKey', 'label'] as const) text(value[key], `packets[${index}].${key}`)
  if (!Array.isArray(value.scopePaths) || value.scopePaths.length === 0) throw new Error(`packets[${index}].scopePaths must be non-empty`)
  if (!Array.isArray(value.evidenceChunks)) throw new Error(`packets[${index}].evidenceChunks must be an array`)
  if (!Array.isArray(value.riskFlags) || value.riskFlags.some(flag => flag !== 'routing-high')) throw new Error(`packets[${index}].riskFlags is invalid`)
  if (typeof value.requirementsPresent !== 'boolean' || typeof value.testEvidencePresent !== 'boolean') throw new Error(`packets[${index}] evidence flags must be booleans`)
  ...
  if (typeof value.budget !== 'object' || value.budget === null || ![value.budget.maxBytes, value.budget.maxLines, value.budget.maxLineChars].every(number => Number.isSafeInteger(number) && number > 0)) throw new Error(`packets[${index}].budget is invalid`)
  return value
}

function paths(value: ReviewPacketMetadata): readonly string[] {
  return [value.packetPath, ...value.evidenceChunks.map(chunk => chunk.path)]
}

function completed(result: WorkflowTaskResult | null, role: string): WorkflowTaskResult {
  if (result === null || result.status !== 'completed') throw new Error(`${role} did not produce an accepted completed result`)
  if (result.verification !== undefined && !result.verification.ok) throw new Error(`${role} did not satisfy its packet-read contract`)
  if (result.structured === undefined) throw new Error(`${role} did not return structured output`)
  return result
}

function rawReview(value: unknown, requirementsPresent: boolean): RawReview {
  const candidate = value as Partial<RawReview> | null
  if (candidate === null || !['compliant', 'issues', 'not-verifiable'].includes(candidate.specVerdict ?? '') || !['approved', 'needs-fixes'].includes(candidate.qualityVerdict ?? '') || !Array.isArray(candidate.findings) || !Array.isArray(candidate.unverifiedRequirements)) throw new Error('primary review returned invalid structured output')
  if (!requirementsPresent && candidate.specVerdict !== 'not-verifiable') throw new Error('a packet without requirements must use specVerdict not-verifiable')
  return candidate as RawReview
}

function findingId(contentHash: string, location: string, claim: string): string {
  return createHash('sha256').update(`${contentHash}\0${location.replaceAll('\\', '/')}\0${claim}`).digest('hex')
}

function normalizeReview(contentHash: string, raw: RawReview): { readonly specVerdict: SpecVerdict; readonly qualityVerdict: QualityVerdict; readonly findings: readonly ReviewFinding[]; readonly unverifiedRequirements: readonly string[] } {
...
function merge(reviews: readonly ReturnType<typeof normalizeReview>[]): ReturnType<typeof normalizeReview> {
  if (reviews.length === 0) throw new Error('at least one primary result is required')
  const variants = new Map<string, ReviewFinding[]>()
  for (const review of reviews) for (const finding of review.findings) variants.set(finding.findingId, [...(variants.get(finding.findingId) ?? []), finding])
...
async function primary(wf: WorkflowApi, value: ReviewPacketMetadata, args: ScopedReviewArgs, second: boolean): Promise<ReturnType<typeof normalizeReview>> {
...
async function verify(wf: WorkflowApi, value: ReviewPacketMetadata, review: ReturnType<typeof merge>): Promise<VerifiedPacketReview> {
  ...
  const dispositions = (result.structured as { readonly findings?: unknown }).findings
  if (!Array.isArray(dispositions)) throw new Error('finding verifier returned invalid structured output')
  const typed = dispositions as FindingDisposition[]
  const expected = new Set(review.findings.map(finding => finding.findingId))
  if (typed.length !== expected.size || typed.some(item => !expected.delete(item.findingId)) || expected.size > 0) throw new Error('finding verifier must dispose every candidate exactly once')
  ...
  const actionable = review.findings.flatMap(finding => {
    ...
    if (disposition.disposition === 'refuted') return []
...
async function reviewPacket(wf: WorkflowApi, value: ReviewPacketMetadata, args: ScopedReviewArgs): Promise<ScopedReviewResult['packetResults'][number]> {
  const reviews = [await primary(wf, value, args, false)]
  if (value.riskFlags.includes('routing-high')) reviews.push(await primary(wf, value, args, true))
...
async function run(wf: WorkflowApi, unknownArgs: unknown): Promise<ScopedReviewResult> {
  const args = unknownArgs as Partial<ScopedReviewArgs> | null
  if (args === null || !Array.isArray(args.packets) || args.packets.length === 0) throw new Error('scoped-review requires a non-empty packets array')
  const packets = args.packets.map(packet)
  const packetResults = await wf.parallel(packets.map(value => async () => await reviewPacket(wf, value, args as ScopedReviewArgs)), { concurrency: 4 })
  if (packetResults.some(result => result === null)) throw new Error('at least one review packet failed; no aggregate verdict was accepted')
  const completedResults = packetResults as ScopedReviewResult['packetResults']
  ...
  const summary = (final.structured as { readonly summary?: unknown }).summary
  if (typeof summary !== 'string') throw new Error('final review synthesis omitted summary')
  return { summary, packetResults: completedResults }
}

export const scopedReviewWorkflow: WorkflowModule = {
