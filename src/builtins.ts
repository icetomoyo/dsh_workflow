...
import type { WorkflowModule, WorkflowPatternId } from './types.js'
import { scopedReviewWorkflow } from './scoped-review.js'

function module(name: string, description: string, phases: readonly string[], maxAgents: number, patterns: readonly WorkflowPatternId[], source: string): WorkflowModule {
  return {
...
const parallelInvestigation = module(
  'parallel-investigation',
  'Investigate a question from independent angles, then synthesize evidence.',
...
const patterns: readonly WorkflowModule[] = [
...
export function listBuiltinWorkflows(): readonly WorkflowModule[] {
  return [parallelInvestigation, scopedReviewWorkflow]
}

export function listWorkflowPatterns(): readonly WorkflowModule[] {
  return patterns
}
