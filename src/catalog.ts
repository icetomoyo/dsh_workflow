...
import type {
  WorkflowCapsule, WorkflowCatalogEntry, WorkflowExecution, WorkflowManifest,
  WorkflowModule, WorkflowSource,
} from './types.js'
...
const TRUSTED_SUFFIXES = ['.ts', '.mjs', '.js'] as const
...
function sourceEntry(module: WorkflowModule, source: 'built-in' | 'pattern'): WorkflowCatalogEntry {
  return {
...
function suffixOf(file: string): string | undefined {
  if (file.endsWith(CAPSULE_SUFFIX)) return CAPSULE_SUFFIX
  return TRUSTED_SUFFIXES.find(suffix => file.endsWith(suffix))
}

function nameOf(file: string, suffix: string): string {
...
async function discoverDirectory(
  ...
  const candidates = new Map<string, { file: string; suffix: string }>()
  for (const file of files.sort()) {
    ...
    if (previous === undefined || suffix === CAPSULE_SUFFIX) candidates.set(name, { file, suffix })
  }
  ...
  for (const [name, candidate] of [...candidates].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    try {
      ...
      if (stat.size > options.maxCapsuleBytes) throw new Error(`workflow file exceeds ${options.maxCapsuleBytes} bytes`)
      if (candidate.suffix === CAPSULE_SUFFIX) {
        const capsule = validateWorkflowCapsule(JSON.parse(await readFile(path, 'utf8')), options)
        if (capsule.manifest.name !== name) throw new Error(`manifest.name "${capsule.manifest.name}" must match filename "${name}"`)
        entries.push({
          name,
          source,
...
export async function discoverWorkflowCatalog(options: WorkflowCatalogOptions): Promise<WorkflowCatalog> {
  ...
  for (const module of options.builtins ?? []) selected.set(module.manifest.name, sourceEntry(module, 'built-in'))
  for (const module of options.patterns ?? []) if (!selected.has(module.manifest.name)) selected.set(module.manifest.name, sourceEntry(module, 'pattern'))
  for (const entry of personal) if (!selected.has(entry.name)) selected.set(entry.name, entry)
  for (const entry of project) {
    const existing = selected.get(entry.name)
    if (existing?.source === 'built-in' || existing?.source === 'pattern') continue
...
async function importTrusted(path: string): Promise<Record<string, unknown>> {
  const stat = await lstat(path)
  ...
  return await import(extname(path) === '.ts' ? url : `${url}?mtime=${String(stat.mtimeMs)}`) as Record<string, unknown>
}

function trustedManifest(value: unknown, limits: Pick<WorkflowCatalogOptions, 'maxAgents' | 'maxConcurrency'>): WorkflowManifest {
  if (typeof value !== 'object' || value === null) throw new Error('trusted workflow meta must be an object')
...
export async function loadWorkflowByName(options: WorkflowCatalogOptions, name: string, allowTrustedLocal = false): Promise<LoadedWorkflow> {
  if (!SAFE_NAME.test(name)) throw new Error(`invalid workflow name "${name}"`)
  const builtin = options.builtins?.find(module => module.manifest.name === name)
  if (builtin !== undefined) return { module: builtin, source: 'built-in', execution: 'trusted-package' }
  const pattern = options.patterns?.find(module => module.manifest.name === name)
  if (pattern !== undefined) return { module: pattern, source: 'pattern', execution: 'trusted-package' }
  ...
  if (entry === undefined) throw new Error(`workflow "${name}" was not found`)
  if (!entry.valid || entry.path === undefined) throw new Error(`workflow "${name}" is invalid: ${entry.error ?? 'unknown error'}`)
  ...
  if (!allowTrustedLocal) throw new Error(`workflow "${name}" is trusted-local and requires explicit approval before loading`)
  const imported = await importTrusted(entry.path)
  const candidate = (imported.default ?? imported) as Record<string, unknown>
  const run = candidate.run
  if (typeof run !== 'function') throw new Error(`trusted workflow "${name}" must export a run function`)
  const manifest = trustedManifest(candidate.manifest ?? candidate.meta, options)
  if (manifest.name !== name) throw new Error(`trusted workflow name "${manifest.name}" must match filename "${name}"`)
  return {
    module: { manifest, execution: 'trusted-local', run: run as NonNullable<WorkflowModule['run']> },
    source: entry.source,
...
function safeTarget(directory: string, name: string): string {
  if (!SAFE_NAME.test(name)) throw new Error(`invalid workflow name "${name}"`)
  const root = resolve(directory)
  const target = resolve(root, `${name}${CAPSULE_SUFFIX}`)
  if (!target.startsWith(`${root}${sep}`)) throw new Error('workflow path escaped its catalog directory')
  return target
}
...
async function ensureDirectoryPath(directory: string, create: boolean): Promise<string> {
  ...
  for (const segment of segments) {
    ...
    if (!stat.isDirectory()) throw new Error(`catalog path component is not a directory: ${current}`)
...
async function writableTarget(directory: string, name: string, createRoot: boolean): Promise<{ root: string; target: string }> {
  if (!SAFE_NAME.test(name)) throw new Error(`invalid workflow name "${name}"`)
...
async function assertSafeLeaf(path: string): Promise<boolean> {
  try {
    const stat = await lstat(path)
    if (stat.isSymbolicLink()) throw new Error(`workflow path is a symbolic link or reparse point: ${path}`)
    if (!stat.isFile()) throw new Error(`workflow path is not a regular file: ${path}`)
...
async function ensureArchiveDirectory(root: string): Promise<string> {
  const archive = await ensureDirectoryPath(join(root, '.archive'), true)
  if (!isWithin(root, archive)) throw new Error('workflow archive escaped its canonical catalog directory')
  return archive
}

export async function saveWorkflowCapsule(directory: string, name: string, capsule: WorkflowCapsule, replace = false): Promise<string> {
  const validated = validateWorkflowCapsule(capsule)
  if (validated.manifest.name !== name) throw new Error('saved workflow name must match manifest.name')
  const { root, target } = await writableTarget(directory, name, true)
  const exists = await assertSafeLeaf(target)
  if (exists && !replace) throw new Error(`saved workflow "${name}" already exists`)
...
export async function deleteSavedWorkflow(directory: string, name: string): Promise<void> {
  const { target } = await writableTarget(directory, name, false)
  await assertSafeLeaf(target)
  await rm(target)
}

export async function renameSavedWorkflow(directory: string, from: string, to: string): Promise<string> {
