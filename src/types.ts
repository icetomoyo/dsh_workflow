...
export type WorkflowJsonSchema =
  | WorkflowInputSchema
  | { readonly type: 'array'; readonly items: WorkflowJsonSchema }
  | { readonly type: 'string'; readonly enum?: readonly string[]; readonly const?: string }
  | { readonly type: 'number' | 'integer' | 'boolean' | 'json'; readonly const?: JsonPrimitive }
...
export interface WorkflowSpawnAgentInput {
  ...
  readonly target?: { readonly agentId: string; readonly expectedConfigurationRevision?: string }
...
export interface WorkflowApi {
  ...
  wait(taskId: string, options?: { readonly timeoutMs?: number }): Promise<WorkflowTaskResult>
  ...
  parallel<T>(thunks: readonly (() => Promise<T>)[], options?: { readonly concurrency?: number }): Promise<(T | null)[]>
  pipeline<T>(items: readonly T[], ...stages: readonly ((value: unknown, item: T, index: number) => Promise<unknown>)[]): Promise<(unknown | null)[]>
  synthesize(input: { readonly inputs: unknown; readonly rubric: string }): Promise<WorkflowSynthesis>
  workflow(name: string, args?: unknown): Promise<unknown>
  artifact(name: string, value: unknown): Promise<WorkflowArtifactRef>
  log(event: string | { readonly message: string; readonly data?: unknown }): void
}
...
export type WorkflowEventType =
  | 'workflow-started' | 'workflow-log' | 'workflow-completed' | 'workflow-failed' | 'workflow-stopped'
  | 'phase-started' | 'phase-completed' | 'agent-started' | 'agent-completed'
  | 'agent-message' | 'synthesis-started' | 'synthesis-completed' | 'nested-started' | 'nested-completed'
...
export interface WorkflowOutcome {
  ...
  readonly errors: readonly { readonly taskId?: string; readonly name?: string; readonly message: string }[]
  readonly usage: WorkflowTaskUsage & { readonly totalSpawned: number }
}
...
export interface WorktreeIsolationAdapter {
  readonly name: string
  prepare(input: { readonly runId: string; readonly taskId: string; readonly cwd: string; readonly parent: Agent }): Promise<{ readonly cwd: string; readonly parent: Agent; dispose(): Promise<void> }>
}
