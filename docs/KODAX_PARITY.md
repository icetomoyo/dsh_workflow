...
| Human UX | `/workflow` list/create/run/runs/show/pause/resume/stop/delete/prune/rerun/save/rename/revise/help and Git review bridge | Same command surface via `ctx.commands`; `/workflow review` captures default/base/commit diffs, requirements and reported test evidence into immutable packets before starting the built-in (no DSH core patch required). Starts return immediately by default, `--wait` is explicit, show defaults to latest, and stop defaults to the active run |
